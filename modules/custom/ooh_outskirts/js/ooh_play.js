...
function passiveBehaviorPreviewLabel() {
 ...
 function syncGeneratedContactPresenceReadout(encounter) {
  ...
  function localTelemetryPulseLines(routeId, pathKey, mode) {
    const routeLines = {
      aer: ['CLOUDLINE ECHO STABLE', 'ALTITUDE VARIANCE: LOCAL', 'SKY CHANNEL HOLDING', 'LOCAL ECHO QUIET', 'SIGNAL VARIANCE: LOW', 'CLOUDLINE ECHO DRIFT', 'ALTITUDE CHANNEL HOLDING'],
      mare: ['PRESSURE RIPPLE: LOCAL', 'CURRENT ECHO LOCAL', 'OXYGEN CHANNEL HOLDING', 'CURRENT HOLDING', 'PRESSURE INDEX SHIFT', 'CURRENT VECTOR DRIFT', 'DEPTH CHANNEL HOLDING'],
      terra: ['DUST INDEX SHIFT', 'GROUND ECHO LOCAL', 'RUIN VISIBILITY VARIANCE', 'FIELD STABLE', 'TERRAIN ECHO LOCAL', 'GROUND SIGNAL FLICKER', 'RUIN SIGNAL HOLDING']
    };
    const pathLines = {
      DOOMED: ['CHANNEL DRIFT: ELEVATED', 'PRESSURE ECHO PASSING', 'PASSIVE SIGNAL HOLD', 'VOLATILE CHANNEL DRIFT', 'VOLATILE ECHO PASSING'],
      MERGED: ['SYNTHETIC CHANNEL CLEAN', 'DISPLAY CHANNEL CLEAN', 'CHANNEL STABLE', 'ECHO ALIGNMENT HOLDING', 'SYNTHETIC ECHO HOLDING']
    };
