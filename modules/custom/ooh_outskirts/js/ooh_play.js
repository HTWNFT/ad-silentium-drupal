...
function passiveBehaviorPreviewLabel() {
 ...
 function syncGeneratedContactPresenceReadout(encounter) {
  ...
  function localTelemetryPulseLines(routeId, pathKey, mode) {
    const routeLines = {
      aer: ['CLOUDLINE ECHO STABLE', 'LOCAL ECHO QUIET', 'ALTITUDE VARIANCE: LOCAL', 'SKY CHANNEL HOLDING', 'CLOUDLINE ECHO DRIFT', 'ALTITUDE CHANNEL HOLDING', 'SIGNAL VARIANCE: LOW'],
      mare: ['CURRENT ECHO LOCAL', 'CURRENT HOLDING', 'PRESSURE RIPPLE: LOCAL', 'OXYGEN CHANNEL HOLDING', 'PRESSURE INDEX SHIFT', 'DEPTH CHANNEL HOLDING', 'CURRENT VECTOR DRIFT'],
      terra: ['GROUND ECHO LOCAL', 'FIELD STABLE', 'DUST INDEX SHIFT', 'RUIN VISIBILITY VARIANCE', 'TERRAIN ECHO LOCAL', 'RUIN SIGNAL HOLDING', 'GROUND SIGNAL FLICKER']
    };
    const pathLines = {
      DOOMED: ['PASSIVE SIGNAL HOLD', 'CHANNEL DRIFT: ELEVATED', 'PRESSURE ECHO PASSING', 'VOLATILE CHANNEL DRIFT', 'VOLATILE ECHO PASSING'],
      MERGED: ['CHANNEL STABLE', 'SYNTHETIC CHANNEL CLEAN', 'DISPLAY CHANNEL CLEAN', 'ECHO ALIGNMENT HOLDING', 'SYNTHETIC ECHO HOLDING']
    };
