$path = "C:\XAMPP11032025\htdocs\STIKWALLET11202025\modules\custom\ooh_outskirts\src\Plugin\Block\OohLandingBlock.php"
...
TXT,
  ...
  public function build(): array {
    ...
    $markup = $this->renderMarkup($slides);
  ...
  private function loadSlidesFromManifests(): array {
    ...
    foreach ($scenario_dirs as $dir) {
      ...
      $title = trim((string) ($manifest['title'] ?? $this->humanizeScenario($scenario)));
      $href = '/' . $scenario;

      $manifest_slides = $manifest['slides'] ?? [];
      if (!is_array($manifest_slides)) {
        $manifest_slides = [];
      }

      $hero = trim((string) ($manifest['hero'] ?? ''));
      if ($hero !== '') {
        array_unshift($manifest_slides, $hero);
      }

      $manifest_slides = array_values(array_unique(array_filter(array_map(
        static fn ($value): string => trim((string) $value),
        $manifest_slides
      ))));

      foreach ($manifest_slides as $filename) {
        $slides[] = [
          'src' => 'adsilentium/loops/' . $scenario . '/' . $filename,
          'href' => $href,
          'title' => 'AD SILENTIUM',
          'tagline' => $title,
          'scenario' => $scenario,
